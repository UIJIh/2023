...
void CHole::hitBy(CSphere& ball) {
	if (this->hasIntersected(ball) && !ball.disabled()) {

		ball.setPosition(100000, ball.getPosition().y, 100000); // out of the display (no rendering)
		ball.setPower(0.0f, 0.0f);
		ball.deactivate();
	}
}

bool CHole::hit(CSphere& ball) {
	return this->hasIntersected(ball);
}
