...
#define FVF_VERTEX    D3DFVF_XYZ|D3DFVF_NORMAL|D3DFVF_TEX1  // direct3d

struct _VERTEX {
	D3DXVECTOR3 pos;  // of vertex
	D3DXVECTOR3 norm; 
	float tu; // texture coordinates - u
	float tv; 
};

const float CSphere::ballRadius = 0.14f;
...
CSphere::CSphere(const char* ballImageFileName) {
	ZeroMemory(&m_mtrl, sizeof(m_mtrl)); // initializing to 0
	this->m_radius = ballRadius;
	this->m_velocity_x = 0;
	this->m_velocity_z = 0;
	this->m_pSphereMesh = nullptr;
	D3DXMatrixIdentity(&m_mLocal);
	D3DXMatrixIdentity(&this->ballRoll);
	this->ballImageName = ballImageFileName;

	int tempBallType = atoi(ballImageFileName);
	if (tempBallType == 0) this->ballType = BallType::QUE;
	else if (tempBallType > 0 && tempBallType < 8) this->ballType = BallType::SOLID;
	else if (tempBallType == 8) this->ballType = BallType::BLACK;
	else if (tempBallType > 8 && tempBallType < 16) this->ballType = BallType::STRIPE;
}


//CSphere::CSphere(BallType ballType, const char* ballImage)
//{
//	ZeroMemory(&m_mtrl, sizeof(m_mtrl)); // memset을 통해 모두 0으로 초기화
//	this->m_radius = CSphere::ballRadius;
//	this->m_velocity_x = 0;
//	this->m_velocity_z = 0;
//	this->m_pSphereMesh = nullptr;
...
bool CSphere::create(IDirect3DDevice9* pDevice) {
	...
	string filePath = "./image/" + this->ballImageName + ".bmp";
...
void CSphere::ballUpdate(float timeDiff) { // center coordinate of the ball
	...
	if (rate < 0) rate = 0;
	this->setPower(getVelocity_X() * rate, getVelocity_Z() * rate); // make slower when moving
}
...
float CSphere::getRadius(void)  const { return (this->m_radius); }
...
void CSphere::deactivate() noexcept {
	this->disableTurn = status.getCurrentTurnCount();
}

void CSphere::activate() noexcept {
	this->disableTurn = -1;
}
...
bool CSphere::disabled() const noexcept {
