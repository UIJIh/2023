...
Player* Status::getNotTurnPlayer() noexcept {
	...
	vector<Player*>::iterator foundIndex 
		= find_if(this->playerList.begin(), this->playerList.end(), [nowPlayer](Player* pPlayer) {
...
void Status::setTurnPlayer(int playerID)
{
	...
	if (foundIndex == this->playerList.end()) {
		throw PlayerNotFoundException("Failed to find the player.");
	}
...
int Status::getWinnerPlayer() const {
	if (winnerPlayer < 0) {
		throw PlayerNotFoundException("Game is not over.");
	}
